...
const popupsManager = (() => {
    const createPopupSection = (popupId, popupTitle, closeBtnId, popupForm) => {
        const popupTitleH3 = domElements.createH('',['popup-titles'],popupTitle,3);
        const popupCloseBtn = domElements.createButton(closeBtnId,['popup-close-btns'],'X');
        const popupContainer = domElements.createDiv('',['popup-containers'],[popupTitleH3, popupCloseBtn, popupForm]);
        const popup = domElements.createSection(popupId,['popups'],[popupContainer]);
        return popup;
    }
...
const createPopup = (() => {
    const newProjectPopup = () => {
        let inputs = [];
        const titleInput = domForm.createTextInput('Project title:',['popup-labels'],'new-project-title', 'new-project-title', 20, ['popup-inputs', 'popup-inputs-text']);
        const descTextarea = domForm.createTextarea('Project description:',['popup-labels','popup-labels-textarea'],'new-project-desc', 'new-project-desc', 100, ['popup-inputs', 'popup-textareas']);
        inputs.push(titleInput, descTextarea);
        const newProjectForm = domForm.createForm('new-project-form', ['popup-forms'], 'Create a new project', ['popup-submits'], inputs); 
    ...
    const newTaskPopup = (projId) => {
        ...
        const newTaskForm = domForm.createForm('new-task-form', ['popup-forms'], 'Create a new task', ['popup-submits'], inputs);
